...
/**
 *	JpaRepository for the CFIntJpaSubProject entities defined in server.markhome.mcf.v3_1.cfint.cfint.jpa.
 *	The manufactured repositories try to provide a rich, do-it-all interface to the JPA data store, with both object and argument-based implementations of the interface defined.
 */
@Transactional(readOnly = true)
public interface CFIntJpaSubProjectRepository extends JpaRepository<CFIntJpaSubProject, ICFLibKeyHash256> {

	@Transactional
	@Modifying
	CFIntJpaSubProject save(CFIntJpaSubProject obj);
	...
	CFIntJpaSubProject get(@Param("id") ICFLibKeyHash256 requiredId);
	...
	List<CFIntJpaSubProject> findByTenantIdx(@Param("tenantId") ICFLibKeyHash256 requiredTenantId);
	...
	List<CFIntJpaSubProject> findByTopProjectIdx(@Param("topProjectId") ICFLibKeyHash256 requiredTopProjectId);
	...
	CFIntJpaSubProject findByNameIdx(@Param("topProjectId") ICFLibKeyHash256 requiredTopProjectId,
	...
	@Query("select r from CFIntJpaSubProject r where r.requiredId = :id")
	CFIntJpaSubProject lockByIdIdx(@Param("id") ICFLibKeyHash256 requiredId);
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFIntJpaSubProject r where r.requiredTenantId = :tenantId")
	List<CFIntJpaSubProject> lockByTenantIdx(@Param("tenantId") ICFLibKeyHash256 requiredTenantId);
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFIntJpaSubProject r where r.requiredContainerParentTPrj.requiredId = :topProjectId")
	List<CFIntJpaSubProject> lockByTopProjectIdx(@Param("topProjectId") ICFLibKeyHash256 requiredTopProjectId);
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFIntJpaSubProject r where r.requiredContainerParentTPrj.requiredId = :topProjectId and r.requiredName = :name")
	CFIntJpaSubProject lockByNameIdx(@Param("topProjectId") ICFLibKeyHash256 requiredTopProjectId,
		@Param("name") String requiredName);
	...
	void deleteByTopProjectIdx(@Param("topProjectId") ICFLibKeyHash256 requiredTopProjectId);
	...
	void deleteByNameIdx(@Param("topProjectId") ICFLibKeyHash256 requiredTopProjectId,
		@Param("name") String requiredName);
