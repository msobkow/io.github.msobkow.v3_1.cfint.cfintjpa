// Description: Java 25 JPA implementation of TopProject history objects
...
/**
 *  CFIntJpaTopProjectH provides history objects matching the CFIntTopProject change history.
...
@Entity
@Table(
    name = "tprjdef_h", schema = "CFInt31",
    indexes = {
        @Index(name = "TopProjectIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "TopProjectTenantIdx_h", columnList = "TenantId", unique = false),
        @Index(name = "TopProjectTopDomainIdx_h", columnList = "TopDomainId", unique = false),
        @Index(name = "TopProjectNameIdx_h", columnList = "TopDomainId, safe_name", unique = false)
    }
)
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFIntPU")
public class CFIntJpaTopProjectH
    implements ICFIntTopProjectH, Comparable<Object>, Serializable
{
    ...
    @EmbeddedId
    protected CFIntJpaTopProjectHPKey pkey;
	@AttributeOverrides({
		@AttributeOverride( name="bytes", column = @Column( name="CreatedByUserId", nullable=false, length=CFLibDbKeyHash256.HASH_LENGTH ) )
	})
	protected CFLibDbKeyHash256 createdByUserId = CFLibDbKeyHash256.fromHex(ICFInt$emitScopingMidfix$TopProject.S_INIT_CREATED_BY);
	...
	protected CFLibDbKeyHash256 updatedByUserId = CFLibDbKeyHash256.fromHex(ICFInt$emitScopingMidfix$TopProject.S_INIT_UPDATED_BY);
    ...
    public CFIntJpaTopProjectH() {
            // The primary key member attributes are initialized on construction
            pkey = new CFIntJpaTopProjectHPKey();
    ...
    @Override
    public ICFIntTopProjectHPKey getPKey() {
        return( pkey );
    }

    @Override
    public void setPKey( ICFIntTopProjectHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFIntJpaTopProjectHPKey) {
                this.pkey = (CFIntJpaTopProjectHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFIntJpaTopProjectHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFIntJpaTopProjectH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
