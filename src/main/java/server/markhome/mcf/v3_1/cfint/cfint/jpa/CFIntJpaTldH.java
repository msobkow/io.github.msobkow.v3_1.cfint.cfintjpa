// Description: Java 25 JPA implementation of Tld history objects
...
/**
 *  CFIntJpaTldH provides history objects matching the CFIntTld change history.
...
@Entity
@Table(
    name = "tlddef_h", schema = "CFInt31",
    indexes = {
        @Index(name = "TldIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "TldTenantIdx_h", columnList = "TenantId", unique = false),
    ...
    implements ICFIntTldH, Comparable<Object>, Serializable
{
    ...
    @EmbeddedId
    protected CFIntJpaTldHPKey pkey;
	@AttributeOverrides({
		@AttributeOverride( name="bytes", column = @Column( name="CreatedByUserId", nullable=false, length=CFLibDbKeyHash256.HASH_LENGTH ) )
	})
	protected CFLibDbKeyHash256 createdByUserId = CFLibDbKeyHash256.fromHex(ICFInt$emitScopingMidfix$Tld.S_INIT_CREATED_BY);
	...
	protected CFLibDbKeyHash256 updatedByUserId = CFLibDbKeyHash256.fromHex(ICFInt$emitScopingMidfix$Tld.S_INIT_UPDATED_BY);
    ...
    @Override
    public ICFIntTldHPKey getPKey() {
        return( pkey );
    }

    @Override
    public void setPKey( ICFIntTldHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFIntJpaTldHPKey) {
                this.pkey = (CFIntJpaTldHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFIntJpaTldHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFIntJpaTldH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
