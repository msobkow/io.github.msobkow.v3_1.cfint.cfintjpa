// Description: Java 25 JPA implementation of SubProject history objects
...
/**
 *  CFIntJpaSubProjectH provides history objects matching the CFIntSubProject change history.
...
@Entity
@Table(
    name = "sprjdef_h", schema = "CFInt31",
    indexes = {
        @Index(name = "SubProjectIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "SubProjectTenantIdx_h", columnList = "TenantId", unique = false),
        @Index(name = "SubProjectTopProjectIdx_h", columnList = "TopProjectId", unique = false),
    ...
    implements ICFIntSubProjectH, Comparable<Object>, Serializable
{
    ...
    @EmbeddedId
    protected CFIntJpaSubProjectHPKey pkey;
	@AttributeOverrides({
    ...
    public CFIntJpaSubProjectH() {
            // The primary key member attributes are initialized on construction
            pkey = new CFIntJpaSubProjectHPKey();
    ...
    @Override
    public ICFIntSubProjectHPKey getPKey() {
        return( pkey );
    }

    @Override
    public void setPKey( ICFIntSubProjectHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFIntJpaSubProjectHPKey) {
                this.pkey = (CFIntJpaSubProjectHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFIntJpaSubProjectHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFIntJpaSubProjectH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
