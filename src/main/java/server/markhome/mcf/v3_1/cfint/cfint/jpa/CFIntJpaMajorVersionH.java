// Description: Java 25 JPA implementation of MajorVersion history objects
...
/**
 *  CFIntJpaMajorVersionH provides history objects matching the CFIntMajorVersion change history.
...
@Entity
@Table(
    name = "mjvrdef_h", schema = "CFInt31",
    indexes = {
        @Index(name = "MajorVersionIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "MajorVersionTenantIdx_h", columnList = "TenantId", unique = false),
        @Index(name = "MajorVersionSubProjectIdx_h", columnList = "SubProjectId", unique = false),
        @Index(name = "MajorVersionNameIdx_h", columnList = "SubProjectId, safe_name", unique = false)
    }
)
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFIntPU")
public class CFIntJpaMajorVersionH
    implements ICFIntMajorVersionH, Comparable<Object>, Serializable
{
	@AttributeOverrides({
    ...
    @EmbeddedId
    protected CFIntJpaMajorVersionHPKey pkey;
	@AttributeOverrides({
		@AttributeOverride( name="bytes", column = @Column( name="CreatedByUserId", nullable=false, length=CFLibDbKeyHash256.HASH_LENGTH ) )
	})
	protected CFLibDbKeyHash256 createdByUserId = CFLibDbKeyHash256.fromHex(ICFInt$emitScopingMidfix$MajorVersion.S_INIT_CREATED_BY);
	...
	protected CFLibDbKeyHash256 updatedByUserId = CFLibDbKeyHash256.fromHex(ICFInt$emitScopingMidfix$MajorVersion.S_INIT_UPDATED_BY);
	...
	@AttributeOverrides({
		@AttributeOverride(name="bytes", column = @Column( name="SubProjectId", nullable=false, length=CFLibDbKeyHash256.HASH_LENGTH ) )
	})
    ...
    public CFIntJpaMajorVersionH() {
            // The primary key member attributes are initialized on construction
            pkey = new CFIntJpaMajorVersionHPKey();
    ...
    @Override
    public ICFIntMajorVersionHPKey getPKey() {
        return( pkey );
    }

    @Override
    public void setPKey( ICFIntMajorVersionHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFIntJpaMajorVersionHPKey) {
                this.pkey = (CFIntJpaMajorVersionHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFIntJpaMajorVersionHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFIntJpaMajorVersionH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
