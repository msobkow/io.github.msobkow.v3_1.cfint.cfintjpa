// Description: Java 25 JPA implementation of TopDomain history objects
...
/**
 *  CFIntJpaTopDomainH provides history objects matching the CFIntTopDomain change history.
...
@Entity
@Table(
    name = "tdomdef_h", schema = "CFInt31",
    indexes = {
        @Index(name = "TopDomainIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "TopDomainTenantIdx_h", columnList = "TenantId", unique = false),
        @Index(name = "TopDomainTldIdx_h", columnList = "TldId", unique = false),
        @Index(name = "TopDomNameIdx_h", columnList = "TldId, safe_name", unique = false)
    }
)
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFIntPU")
public class CFIntJpaTopDomainH
    implements ICFIntTopDomainH, Comparable<Object>, Serializable
{
    ...
    @EmbeddedId
    protected CFIntJpaTopDomainHPKey pkey;
	@AttributeOverrides({
		@AttributeOverride( name="bytes", column = @Column( name="CreatedByUserId", nullable=false, length=CFLibDbKeyHash256.HASH_LENGTH ) )
	})
	protected CFLibDbKeyHash256 createdByUserId = CFLibDbKeyHash256.fromHex(ICFInt$emitScopingMidfix$TopDomain.S_INIT_CREATED_BY);
	...
	protected CFLibDbKeyHash256 updatedByUserId = CFLibDbKeyHash256.fromHex(ICFInt$emitScopingMidfix$TopDomain.S_INIT_UPDATED_BY);
    ...
    public CFIntJpaTopDomainH() {
            // The primary key member attributes are initialized on construction
            pkey = new CFIntJpaTopDomainHPKey();
    ...
    public void setPKey( ICFIntTopDomainHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFIntJpaTopDomainHPKey) {
                this.pkey = (CFIntJpaTopDomainHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFIntJpaTopDomainHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFIntJpaTopDomainH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
